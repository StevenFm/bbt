...
{
    BOOL			bResult		= false;
    sighandler_t	pHandler	= NULL;

    pHandler = signal(SIGPIPE, SIG_IGN);
    XY_FAILED_JUMP(pHandler != SIG_ERR);

    m_nEventCount = nMaxListen + nMaxStream;
    m_pEventTable = new epoll_event[m_nEventCount];
    XY_FAILED_JUMP(m_pEventTable);

    m_nEpoll = epoll_create(nMaxListen + nMaxStream);
    XY_FAILED_JUMP(m_nEpoll != -1);
...
XSocket* XSocketMgr::Listen(const char cszIP[], int nPort, size_t uMaxRecvPackSize, size_t uRecvBufferSize)
{
    ...
    XYLOG_FAILED_JUMP(nSocket != INVALID_SOCKET);
    ...
    if (uRecvBufferSize > 0)
    {
        ...
        nRetCode = setsockopt(nSocket, SOL_SOCKET, SO_RCVBUFFORCE, (char*)&nBufSize, sizeof(nBufSize));
    ...
    {
        Log(eLogDebug, "bind %d", GetSocketError());
    }
    XYLOG_FAILED_JUMP(nRetCode != SOCKET_ERROR);
...
BOOL XSocketMgr::SetSendBufferSize(XSocket* pSocket, size_t uBufferSize)
{
    ...
    nOptRet = setsockopt(pNode->nSocket, SOL_SOCKET, SO_SNDBUFFORCE, (char*)&uBufferSize, sizeof(uBufferSize));
    return (nOptRet == 0);
}
...
BOOL XSocketMgr::SetRecvBufferSize(XSocket* pSocket, size_t uBufferSize)
{
    ...
    nOptRet = setsockopt(pNode->nSocket, SOL_SOCKET, SO_RCVBUFFORCE, (char*)&uBufferSize, sizeof(uBufferSize));
    return (nOptRet == 0);
}
...
int XSocketMgr::Query()
{
    ...
    nCount = epoll_wait(m_nEpoll, m_pEventTable, m_nEventCount, m_nQueryTimeout);

	for (int i = 0; i < nCount; i++) 
    {
        epoll_event* pEvent = &m_pEventTable[i];

        if (!(pEvent->events & EPOLLIN))
            continue;

        XSocketNode* pNode = (XSocketNode*)pEvent->data.ptr;
...
void XSocketMgr::ProcessCycle(time_t uTimeNow)
{
    ...
    while (pListenNode)
    {
        ...
        Log(eLogDebug, "ProcessCycle close socket:%p", pListenNode);
